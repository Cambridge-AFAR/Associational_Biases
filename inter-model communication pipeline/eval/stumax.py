    ...
    for label, bias_pairs in sorted(pairs.items(), key=lambda item: item[0]):
        for bias_type, categories in bias_vals.items():
            if bias_type in bias_pairs and bias_pairs[bias_type]:
                stat, p, df, n = stuart_maxwell_test(bias_pairs[bias_type], categories)
                tests.append(f"{label} {bias_type}")
                stats.append(stat)
                pvals.append(p)
                dfs.append(df)
                ns.append(n)
                # print(f"  {bias_type}: stat={stat:.2f}, p={p:.2e}")
            else:
                print(f"{task}: {label} {bias_type} has no paired data")

    rejected, corrpvals = fdrcorrection(pvals, alpha=ALPHA)

    string = []

    for i, (t, s, p, pp, r, df, n) in enumerate(
        zip(tests, stats, pvals, corrpvals, rejected, dfs, ns)
    ):
        if i % 3 == 0:
            string.append(t.split(" ")[0].title())
        string.append(str(n))
        string.append(f"\\num{{{p:.1e}}} & \\num{{{pp:.1e}}}")
        if i % 3 == 2:
            string[-1] += r" \\"
    ...
    for emo_id, emo in enumerate(RAF_DB_EMOTIONS, start=1):
        ...
        for bias_type, categories in bias_vals.items():
            if bias_type in emo_pairs and emo_pairs[bias_type]:
                stat, p, df, n = stuart_maxwell_test(emo_pairs[bias_type], categories)
                tests.append(f"{emo} {bias_type}")
                stats.append(stat)
                pvals.append(p)
                dfs.append(df)
                ns.append(n)
...
if __name__ == "__main__":
    ...
    raf_stumax()
    print("\n\n### Phase emotions ###")
    phase_stumax("emotion")
    print("\n\n### Phase activities ###")
    phase_stumax("activity")
